...
CREATE TABLE subprefeitura(
    ...
    fk_empresa INT NOT NULL,
    CONSTRAINT fk_subprefeitura_empresa FOREIGN KEY (fk_empresa) REFERENCES empresa(id_empresa)
);
...
CREATE TABLE ecoponto (
    ...
    CONSTRAINT fk_ecoponto_subprefeitura FOREIGN KEY (fk_subprefeitura) REFERENCES subprefeitura (id_subprefeitura)
);
...
INSERT INTO
    sensor (situacao_sensor, fk_lixeira)
VALUES
    (TRUE, 1),
    (FALSE, 2),
    (TRUE, 3),
...
insert into
    leitura_sensor(nivel_preenchimento, fk_sensor) value (67, 1);

insert into
    leitura_sensor(nivel_preenchimento, fk_sensor)
values
    (68, 1),
    (54, 2),
    (18, 3),
...
FROM
    funcionario u
    INNER JOIN empresa e ON u.fk_empresa = e.id_empresa;
...
CREATE VIEW captura_ultimos_dados_sensor AS
SELECT
    id_sensor,
...
CREATE VIEW captura_nivel_sensor AS
SELECT
    l.nivel_preenchimento,
    l.cadastrado_em,
    DATE_FORMAT(l.cadastrado_em, '%H:%i:%s')
...
-- sensor e bairro
CREATE VIEW captura_ultimos_dados_sensor_tabela AS
SELECT
